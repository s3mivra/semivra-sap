...
import { fetchWarehouses, createWarehouse, fetchStockHistory, recordMovement } from '../services/inventoryService';
...
const AdminInventoryManager = () => {
    ...
    const loadData = async () => {
        try {
            const [whData, prodData, histData] = await Promise.all([
                fetchWarehouses(),
                fetchProducts(),
                fetchStockHistory()
            ]);
        ...
        } catch (error) { console.error('Failed to load inventory data'); } 
                        ...
                        {products.filter(p => p.isPhysical).length === 0 ? (
                            ...
                            products.filter(p => p.isPhysical).map(p => (
                                <tr key={p._id} style={{ borderBottom: '1px solid #eee', transition: 'background-color 0.2s' }} onMouseOver={e => e.currentTarget.style.backgroundColor = '#f8f9fa'} onMouseOut={e => e.currentTarget.style.backgroundColor = 'transparent'}>
                                    ...
                                    <td style={{ padding: '12px', color: '#7f8c8d' }}>{p.category?.name || 'N/A'}</td>
                                    <td style={{ padding: '12px', color: '#7f8c8d' }}>{p.unit?.abbreviation || 'N/A'}</td>
